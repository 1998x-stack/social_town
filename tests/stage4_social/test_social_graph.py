...
def test_record_interaction_is_bidirectional(graph):
    graph.record_interaction("a1", "a2")
    assert graph.get_edge("a2", "a1").interaction_count == 1
    assert graph.get_edge("a2", "a1").intimacy == pytest.approx(0.15, abs=0.01)
